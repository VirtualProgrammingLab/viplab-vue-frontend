...
export function Parse(viplabFile) {
    ...
    this.parseFileToJson = function() {
        var json = JSON.parse(this.file.replace(/\n/g, "\\n"));
        return json;
    }
    ...
    this.parseValues = function(json) {
        ...
        var width = 0, height = 0; // Dimensions of the array.
        var xmin = 0.0, ymin = 0.0, xmax = 0.0, ymax = 0.0; // range on the screen
        var xscale = 1.0, yscale = 1.0, zscale = 1.0; // scaling factors
        ...
        for (var i = 1; i < value.length; i++) {
            if (value[i] != null && (!(value[i] === "\n")) && (!(value[i].startsWith("##")))) {
                ...
                if (!value[i].startsWith("#") && value[i].length > 0) {
                    ...
                    for (var idx = 0; idx < valueArray[valueArray.length - 1].length; idx++) {
                        var z = valueArray[valueArray.length - 1][idx];

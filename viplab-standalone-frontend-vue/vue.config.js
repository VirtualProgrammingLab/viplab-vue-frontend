...
const setParameters = function (filename) {
  ...
  for (const fileIndex in json.files) {
    for (const partIndex in json.files[fileIndex].parts) {
      const currentPart = json.files[fileIndex].parts[partIndex];
      if (currentPart.access === 'template') { // && currentPart.content === "") {
        const contentFile = filename.substring(0, filename.indexOf('.') + 1);
        let content = '';
        try {
